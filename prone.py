...
def nmi(N):
    ...
    nv = b1.dot(N)
    nu = N.dot(b2)
    f = np.vectorize(lambda x: x * math.log(x) if x > 0 else 0)
    Hu = -(b1.dot(f(nu / n)))
    Hv = -(f(nv / n).dot(b2))
...
def ProNe_l(e): return quantify_confusion(e, f=lambda x: x*np.log(x)  if x > 0 else 0)
...
def attribute_mixing_dict(G, attribute1, attribute2, nodes=None, normalized=True, missing_val = 0):
...
def get_all(G,attribute_dict, att, att2, print_mixing = False, normalized= True, times  = False , missing_val= 0):
